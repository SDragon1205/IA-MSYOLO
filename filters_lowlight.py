...
from core.util_filters import lrelu, rgb2lum, tanh_range, lerp
import cv2
import math
...
class DIP:
  def __init__(self, cfg):
    self.filter_imgs_series = []
    self.filters = cfg.filters#cfg.filters = [ImprovedWhiteBalanceFilter,  GammaFilter, ToneFilter, ContrastFilter, UsmFilter]
    self.filter_parameters = []
    self.cfg = cfg

  @tf.function(experimental_relax_shapes=True)
  def __call__(self, filtered_image_batch, filter_features):
    self.filters = [x(filtered_image_batch, self.cfg) for x in self.filters]
    for j, filter in enumerate(self.filters):
        with tf.compat.v1.variable_scope('filter_%d' % j):
            ...
            filtered_image_batch, filter_parameter = filter.apply(
                filtered_image_batch, filter_features)#DIP module
                
            #print('filtered_image_batch: ', type(filtered_image_batch), filtered_image_batch.shape)
            #print('filter_parameter: ', type(filter_parameter), filter_parameter.shape)
            self.filter_parameters.append(filter_parameter)
            self.filter_imgs_series.append(filtered_image_batch)

            #print('      output:', filtered_image_batch.shape)
    self.filter_params = self.filter_parameters
    self.image_isped = filtered_image_batch#經過DIP處理的圖片
    self.filter_imgs_series = self.filter_imgs_series
    
    return filtered_image_batch

# mydip=DIP(cfg)
# image_data = mydip(filtered_image_batch, filter_features)
...
class Filter:
  ...
  def apply(self,
            ...
            high_res=None):#img = input_data, img_features=filter_features
