...
dirPath1 = '/mnt/HDD1/iamsyolo/ia-yolov4-tflite/tsne/ad/config/__pycache__'
dirPath2 = '/mnt/HDD1/iamsyolo/ia-yolov4-tflite/tsne/ad/config/augmentation'

flags.DEFINE_string('model', 'yolov4', 'yolov4, yolov3')
flags.DEFINE_boolean('tiny', True, 'yolo or yolo-tiny')
flags.DEFINE_boolean('qat', False, 'train w/ or w/o quatize aware')
flags.DEFINE_string('save_dir', 'tsne/ad', 'save model dir')
flags.DEFINE_string('weights', './checkpoints/test_ad/ckpt/final.ckpt', 'path to weights file')
flags.DEFINE_string('weights2', './checkpoints/test_ad/cnn_pp_final', 'path to weights file')
...
def main(_argv):
    ...
    **************       ****  ****         ***           **      **
    ...
    def get_tsne_image(image_data, train_target):
        ...
        #print('\ntrain_target[0][0]的new shape: ', train_target[0][0].shape)
        ...
        width = height = 38
        fm_source = np.empty(shape=256)
        for batch in range(batch_size//2):
            for w in range(width):
                for h in range(height):
                    if(tf.math.reduce_sum(feature_masked[batch][w][h]) > 0):
                        fm_source = np.append(fm_source, feature_masked[batch][w][h])
        fm_source = np.reshape(fm_source, (-1, 256))

        fm_target = np.empty(shape=256)
        for batch in range(batch_size//2, batch_size):
            for w in range(width):
                for h in range(height):
                    if(tf.math.reduce_sum(feature_masked[batch][w][h]) > 0):
                        fm_target = np.append(fm_target, feature_masked[batch][w][h])
        fm_target = np.reshape(fm_target, (-1, 256))
        
        return fm_source, fm_target
        #sys.exit()
    ...
    with tqdm(total=len(trainset), ncols=200) as pbar:
        for iter_idx, data_item in enumerate(trainset):
            ...
            feature_masked_source, feature_masked_target = get_tsne_image(images, train_targets)
            print('\nfeature_masked_source 的 shape: ', feature_masked_source.shape)
            print('\nfeature_masked_target 的 shape: ', feature_masked_target.shape)
            print('iter = ', iter_idx, "/", total)
            np.save(f'tsne_images/adversarial/source/image_{iter_idx}', feature_masked_source) #ad
            np.save(f'tsne_images/adversarial/target/image_{iter_idx}', feature_masked_target) #ad
