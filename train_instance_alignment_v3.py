...
dirPath1 = '/mnt/HDD1/iamsyolo/ia-yolov4-tflite/checkpoints/test_ad/config/__pycache__'
dirPath2 = '/mnt/HDD1/iamsyolo/ia-yolov4-tflite/checkpoints/test_ad/config/augmentation'

flags.DEFINE_string('model', 'yolov4', 'yolov4, yolov3')
flags.DEFINE_string('weights', None, 'pretrained weights')
flags.DEFINE_boolean('tiny', True, 'yolo or yolo-tiny')
flags.DEFINE_boolean('qat', False, 'train w/ or w/o quatize aware')
flags.DEFINE_string('save_dir', 'checkpoints/test_ad', 'save model dir')
...
def main(_argv):
   ...
   **            **      *****
    **          **     ***   ***
    ...
    output_maps = YOLO(input_layer, NUM_CLASS, FLAGS.model, FLAGS.tiny, dc_head_type=1) # [1] dc_head_type=2
    ...
    # (num_traffic_lightx256)   = (38x38x256)  * (38x38x1)
    # light_feature = (da_maps[0]).reshape(-1,256)[mask.reshpae(-1) > 0] 


    # print("input_layer: ", input_layer)
    # print("bbox_tensors: ", bbox_tensors[0])
    # sys.exit()
    ...
    def train_step(image_data, train_target):
        ...
        with tf.GradientTape(persistent=True) as tape:
            #filter_params = CNNPP(image_data, training=True)
            #processed_data = DIP(filter_params, image_data)
            #dict_result = model(processed_data, training=True)
            ###################
            input_data_clean = image_data
            filtered_image_batch = image_data
            filter_params = image_data
            filter_imgs_series = []
            if cfg.isp_flag:
                # with tf.compat.v1.variable_scope('extract_parameters_2'):
                filter_features = CNN_PP(input_data, training = True)#CNN-PP module
                # print('filter_features: ', type(filter_features), filter_features.shape)#cfg.num_filter_parameters = 14

                # filter_features = tf.random_normal([1, 10], 0.5, 0.1)
                filtered_image_batch = dip(filtered_image_batch, filter_features)
                ###
            recovery_loss = tf.reduce_sum(tf.pow(filtered_image_batch - input_data_clean, 2.0)) / tf.cast(batch_size, tf.float32)#/(2.0 * batch_size)
            ###############
            dict_result = model(filtered_image_batch, training=True)
            pred_result = [
            ...
            total_loss_cnnpp = total_loss + recovery_loss

        gradients = tape.gradient(total_loss, model.trainable_variables)
        optimizer.apply_gradients(zip(gradients, model.trainable_variables))
        #CNN_PP
        
        gradients2 = tape.gradient(total_loss_cnnpp, CNN_PP.trainable_variables)
        optimizer2.apply_gradients(zip(gradients2, CNN_PP.trainable_variables))
        # del tape
    ...
    def test_step(image_data, train_target):
        batch_size = tf.shape(image_data)[0]
        input_data = tf.image.resize(image_data, [256, 256], method=tf.image.ResizeMethod.BILINEAR)#調整圖片大小為256*256，方法為雙線性插植
        #filter_params = CNNPP(image_data, training=False)
        #processed_data = DIP(filter_params, image_data)
        #dict_result = model(processed_data, training=False)
        ###################
        input_data_clean = image_data
        filtered_image_batch = image_data
        filter_params = image_data
        ...
        total_loss = giou_loss + conf_loss + prob_loss + da_loss
        total_loss_cnn = total_loss + recovery_loss
        return {
            'giou_loss': giou_loss, 
            'conf_loss': conf_loss, 
            'prob_loss': prob_loss,
    ...
    for epoch in range(1, 1+first_stage_epochs + second_stage_epochs):
        ...
        with tqdm(total=len(trainset), ncols=200) as pbar:
            for iter_idx, data_item in enumerate(trainset):
                ...
                re_loss_counter.update(loss_dict['re_loss'], batch_size)
                # debug statistics
                ...
                if (iter_idx + 1) % 200 == 0:
                    ...
                    display_images = (filtered_image_batch*255.0).numpy().astype(np.uint8)
                ...
                total_cnn = giou_loss_counter.get_average() \
                    + conf_loss_counter.get_average() \
                    + prob_loss_counter.get_average() \
                    + da_loss_counter.get_average() \
                    + re_loss_counter.get_average()
                ...
                pbar.set_postfix({
                    ...
                    're_loss': f"{re_loss_counter.get_average():8.6f}",
                    'total': f"{total: 6.4f}",
                    'total_cnn': f"{total_cnn: 6.4f}",
        ...
        if epoch % 5 == 0 or (1+first_stage_epochs + second_stage_epochs - epoch) < 10:
            ...
            with tqdm(total=len(testset), ncols=150, desc=f"{'Test':<13}") as pbar:
                ...
                for data_item in testset:
                    ...
                    # print("test_step(images, train_targets) ", type(test_step(images, train_targets)))
                    # sys.exit()
                    ...
                    pbar.set_postfix({
                        ...
                        're_loss': f"{re_loss_counter.get_average():6.4f}",
                        'total': f"{total: 6.4f}",
                        'total_cnn': f"{total_cnn: 6.4f}"
                    })
                    pbar.update(1)
            ...
            CNN_PP.save_weights(os.path.join(FLAGS.save_dir, 'ckpt', f'cnn_pp_{epoch:04d}.ckpt'))
    ...
    CNN_PP.save_weights(os.path.join(FLAGS.save_dir, 'ckpt', f'cnn_pp_final.ckpt'))
